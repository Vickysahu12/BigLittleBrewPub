...
import Cafe from "../assets/image/cafeshop.webp"; // Make sure file name has no spaces
...
const HeroSection = () => {
  // Preload background image for instant render
  useEffect(() => {
    const img = new Image();
    img.src = Cafe;
  }, []);
            ...
            {[...Array(2)].map((_, idx) => (
              <div key={idx} className="flex items-center gap-12 px-6">
                ...
                <div className="flex items-center gap-2">
                  <img src={location} alt="Location" className="h-4 w-4" loading="lazy" />
                  <span className="text-sm md:text-base">6 Sandgate T015 IEP</span>
                </div>

                {/* Opening Hours */}
                <div className="flex items-center gap-2">
                  <img src={alarm} alt="Opening Hours" className="h-4 w-4" loading="lazy" />
                  <span className="text-sm md:text-base">Mon - Sunday 10AM to 3PM</span>
                </div>

                {/* Contact */}
                <div className="flex items-center gap-2">
                  <img src={viber} alt="Phone" className="h-4 w-4" loading="lazy" />
